...
document.addEventListener('DOMContentLoaded', function () {
    ...
    //For restoring elements after higlighting
    function resetElementsWithIds(classAddId, styleChangeId) {
        const elWithClass = document.getElementById(classAddId);
        if (elWithClass) {
            elWithClass.classList.add('blurred');
        }

        const elWithStyle = document.getElementById(styleChangeId);
        if (elWithStyle) {
            elWithStyle.style.display = 'block';
        }
    }

    function removeElementsWithIds(classAddId, styleChangeId) {
        const elWithClass = document.getElementById(classAddId);
        if (elWithClass) {
            elWithClass.classList.remove('blurred');
        }

        const elWithStyle = document.getElementById(styleChangeId);
        if (elWithStyle) {
            elWithStyle.style.display = 'none';
        }
    }
    ...
    function getStepSelector(step) {
        ...
        return typeof attachTo?.element === 'string' ? attachTo.element : null;
    }
    ...
    tour.addStep({
        id: 'step-3',
        text: 'See your monthly income vs. expenses here.',
        attachTo: {
            element: '#incomeExpensesDiv', // Replace with actual element selector
            on: 'right'
        },
        buttons: [
            {
    ...
    tour.addStep({
        id: 'step-4',
        text: 'Track your total balance over time.',
        attachTo: {
            element: '#totalBalanceDiv', // Replace with actual element selector
            on: 'right'
        },
        buttons: [
            {
    ...
    tour.addStep({
        id: 'step-5',
        text: 'See your latest transactions',
        attachTo: {
            element: '#recentTransactionDiv', // Replace with actual element selector
            on: 'right'
        },
        buttons: [
            {
    ...
    tour.addStep({
        id: 'step-6',
        text: 'View your spending breakdown by category.',
        attachTo: {
            element: '#spendingDiv', // Replace with actual element selector
            on: 'right'
        },
        buttons: [
            {
    ...
    tour.on('show', (step) => {
        currentIndex = step.step.id;

        const currentSelector = getStepSelector(step.step);

        if (lastStepSelector) {
            restoreElement(lastStepSelector);
        }

        // Unblur current one
        if (currentSelector) {
            unblurElement(currentSelector);
            lastStepSelector = currentSelector;
        }

        // step 3
        if (step.step.id === 'step-3') {
            // Remove blurred class from chart div
            removeElementsWithIds('kt_apexcharts_2','chartMessage');
        } else if (step.step.id === 'step-4') {
            removeElementsWithIds('kt_apexcharts_3','balanceChartMessage');

            resetElementsWithIds('kt_apexcharts_2','chartMessage')
        } else if (step.step.id === 'step-5') {
            removeElementsWithIds('table','tableMessage');

            resetElementsWithIds('kt_apexcharts_3','balanceChartMessage')
        } else if (step.step.id === 'step-6') {
            removeElementsWithIds('chartdiv','categoryChartMessage');

            resetElementsWithIds('table','tableMessage')
        }
        else {
            // resetElementsWithIds('kt_apexcharts_2','chartMessage')
            // resetElementsWithIds('kt_apexcharts_3','balanceChartMessage')
            // resetElementsWithIds('table','tableMessage')
            // resetElementsWithIds('chartdiv','categoryChartMessage')
    ...
    tour.on('complete', () => {
        ...
        // restoreElement('#incomeExpensesDiv');
        // resetElementsWithIds('#kt_apexcharts_2','#chartMessage')
        restoreElement('#spendingDiv');
        resetElementsWithIds('chartdiv','categoryChartMessage')
    });
